...
class PPO(object):
	...
	def ComputeTDandGAE(self):
		...
		#print('replay_buffer : {}'.format(self.num_tuple))	#no, this is not the same as local_step, do not mix them up.
	...
	def GenerateTransitions(self):
		...
		while True:
			...
			if local_step >= self.buffer_size:	
				# when the new simulated step count reach 2048, just stop. 
				# But the training will only use the data from terminated episode for training.
				# The unterminated episode will be continued in the next round.
				print('SIM : {}'.format(local_step))	#final number of local step
				break
	...
	def Evaluate(self):
		...
		print('||Num Transition in train  : {}'.format(self.num_tuple))		#in the replay_buffer
		...
		print('||Avg Step per episode     : {:.1f}'.format(self.num_tuple/self.num_episode))  #because max time per episode is just 10 seconds (max step is just 300)
		...
		print('=============================================')
		self.stepPerEpisodeList.append(self.num_tuple/self.num_episode)
		return np.array(self.rewards),np.array(self.stepPerEpisodeList)
...
def Plot(y,stepPerEpisode,title,num_fig=1,ylim=True):
	...
	plt.clf()
	plt.title(title)
	plt.plot(y,'b')	#average from latest set of complete episode
	
	plt.plot(temp_y,'r')	#moving average of last 5 

	plt.plot(stepPerEpisode/10,'g')
...
if __name__=="__main__":
	...
	for i in range(ppo.max_iteration-5):
		ppo.Train()
		rewards,stepPerEpisode = ppo.Evaluate()
		Plot(rewards,stepPerEpisode,'reward',0,False)