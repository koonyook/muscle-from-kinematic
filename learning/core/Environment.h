...
public:
	void Step();	//bool withPerturbation = false);
	...
	double GetWorldTime(){return mWorld->getTime();}
	int GetControlStep(){return mControlStepCount;}	

	//muscle grouping information, important for exporting muscle activation
	int muscleN;
	int *groupIndexFromMuscleIndex;
	std::map<std::string,int> groupIndexFromGroupName;
	std::map<std::string,std::vector<int>> muscleListFromGroupName;
	int muscleGroupN;
	std::string *groupNameFromGroupIndex;

	Eigen::VectorXd convertActivationToGroupActivation(Eigen::VectorXd activation);
