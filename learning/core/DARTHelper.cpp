...
MakeFreeJointProperties(const std::string& name,const Eigen::Isometry3d& parent_to_joint,const Eigen::Isometry3d& child_to_joint)
{
	...
	props->mName = name;
	props->mT_ParentBodyToJoint = parent_to_joint;	//this is from somewhere around the ground to center of pelvis rotation. this cause the root position in mTargetPosition not to be like bvh but to be roughly on the ground.
...
BuildFromFile(const std::string& path,bool create_obj)
{
	...
	if(!doc.LoadFile(path)){
		std::cout << "Can't open skeletal file : " << path << std::endl;
		return nullptr;
	}
	...
	for(TiXmlElement* node = skeleton_elem->FirstChildElement("Node");node != nullptr;node = node->NextSiblingElement("Node"))
	{
		...
		ShapePtr shape;
		Eigen::Isometry3d T_body = Eigen::Isometry3d::Identity(); //isometric transformation = shape preserving (just rot+trans)
		...
		{
			std::string obj_path = path.substr(0, path.size()-9) + "OBJ/"+obj_file;
