...
GetMotion(double t)
{
	...
	if(mCyclic)
		k %= mNumTotalFrames;
	k = std::max(0,std::min(k,mNumTotalFrames-1));	//for noncyclic, k is going to be the last frame if time exceed.
	...
	for(auto ss : mBVHMap)
	{
		...
		if(jn->getType()=="FreeJoint")
		{
			...
			T.linear() = R;
			p.segment<6>(idx) = FreeJoint::convertToPositions(T);	//in rotVec + translation
		}
...
Parse(const std::string& file,bool cyclic)
{
	...
	if(!is)
	{
		std::cout<<"Can't Open BVH File:"<<file<<std::endl;
		return;
	}
	while(is>>buffer)
	{
	...
	w_T0.linear() = this->Get(root_bvh_name);	//confirmed
	w_T0.translation() = 0.01*m.segment<3>(0);	//confirmed
	...
	w_T1.linear() = this->Get(root_bvh_name);	
	w_T1.translation() = 0.01*m.segment<3>(0);

	//calculate the transition for the shift of w_mTc
	Eigen::Isometry3d w_T0projected=ProjectToGround(w_T0);
	Eigen::Isometry3d w_T1projected=ProjectToGround(w_T1);
	
	T0projected_T1projected = w_T0projected.inverse() * w_T1projected;

	//observe T0 to check the convention of rotation matrix
	
	//std::cout<<"T0\n"<<T0.linear()<<"\n====\n";
	//std::cout<<"T0\n"<<T0.translation()<<"\n====\n";
	//for(int i=0;i<9;i++)
	//{
	//	std::cout<<T0.linear()[i]<<"\n";
	//}
}
BVHNode*
BVH::
